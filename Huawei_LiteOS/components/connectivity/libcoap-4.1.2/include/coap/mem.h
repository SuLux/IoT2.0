...
static inline void coap_free(void *pointer) {
	LOS_MemFree(m_aucSysMem0, pointer);
  //LWIP_ASSERT("coap_free must not be used in lwIP", 0);
}

static inline void *coap_malloc_type(coap_memory_tag_t type, size_t size)
{
    return coap_malloc(size);
}
static inline void coap_free_type(coap_memory_tag_t type, void *p)
{
    coap_free(p);
}
